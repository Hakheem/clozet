...
function AuthInput({
  ...
  return (
    <input
      ...
      {...props}
      className="w-full h-10 px-3.5 rounded-md text-sm outline-none transition-all duration-200"
      style={{
  ...
  const onSubmit = async (data: SignUpFormData) => {
    try {
      const { error } = await (signUp.email as any)({
        ...
        name: data.name,
        role: data.role, // Pass directly to better-auth
      });
  ...
  return (
    ...
    >
        ...
        <div className="grid grid-cols-2 gap-2">
          {(["USER", "SELLER"] as const).map((r) => {
            ...
            return (
              <label
                key={r}
                className="relative flex items-center gap-2.5 h-10 px-3.5 rounded-md cursor-pointer transition-all duration-200"
                style={{
      ...
      <motion.button
        ...
        className="cursor-pointer w-full flex items-center justify-center gap-3 h-10 rounded-md transition-all duration-200 text-sm font-medium disabled:opacity-50 disabled:cursor-not-allowed"
        style={{
          background: "rgba(255,255,255,0.04)",
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
        ...
        <motion.div variants={item}>
          <button
            type="submit"
            disabled={isSubmitting || isGoogleLoading}
            className="w-full h-11 rounded-md flex items-center justify-center gap-2 text-sm font-semibold transition-all duration-200 group mt-1 disabled:opacity-50 disabled:cursor-not-allowed"
            style={{
